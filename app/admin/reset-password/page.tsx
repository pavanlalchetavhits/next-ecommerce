...
function AdminResetPasswordContent() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!currentToken || !currentEmail) {
      setGlobalError('Invalid or missing admin reset token. Please click the link sent to your email.');
      return;
    }
  ...
  return (
    <SplitAuthLayout
      heroTitle="Set New Admin Credentials"
      heroSubtitle="Update your store administrator password to regain full access to store operations and analytics."
      heroFeatures={[
        '10-Minute Token Expiration Security',
        '256-Bit Password Encryption',
        'Instant Admin Portal Re-authentication',
      ]}
      icon={<AdminShieldIcon sx={{ color: '#fff', fontSize: 26 }} />}
      title="Reset Admin Password"
      subtitle="Enter a new password for your administrator account below"
    ...
    >
      ...
      {isMissingParams ? (
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5, textAlign: 'center' }}>
          ...
          >
            No valid admin reset token found in URL. Please click the reset password link sent to your registered email address.
          </Alert>
        ...
        <Box component="form" onSubmit={handleSubmit} noValidate sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          <Button
            ...
            sx={{ mt: 1 }}
          >
            {loading ? <CircularProgress size={24} sx={{ color: '#ffffff' }} /> : 'Set New Admin Password'}
          </Button>
